  ...
  if (notFound || !market) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/" className="text-blue-400 hover:underline mt-2 block">Back to Home</Link>
      </div>
    )
  }
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-xl border border-gray-800 bg-gray-900 p-6">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {market.status === 'resolved' && (
                    <Badge variant={market.resolvedOutcome === 'yes' ? 'success' : 'danger'}>
                      Resolved: {market.resolvedOutcome === 'yes' ? 'YES wins' : 'NO wins'}
                    </Badge>
                  )}
                  {market.tags.map(tag => (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Volume', value: formatVolume(market.volume), icon: TrendingUp },
                  { label: 'Liquidity', value: formatVolume(market.liquidity), icon: TrendingUp },
                  { label: 'Participants', value: market.participantCount.toLocaleString(), icon: Users },
                  { label: 'Closes', value: timeUntil(market.endDate), icon: Clock },
                ].map(stat => (
            ...
            <div>
              <div className="flex border-b border-gray-800 mb-4">
                {(['chart', 'comments', 'info'] as const).map(tab => (
                  ...
                  >
                    {tab === 'chart' ? 'Price History' : tab === 'comments' ? `Comments (${comments.length})` : 'Rules'}
                  </button>
                ))}
              </div>
              ...
              {activeTab === 'chart' && (
                <div className="rounded-xl border border-gray-800 bg-gray-900 p-5">
                  ...
                  <div className="flex items-center gap-4 mt-3 text-xs text-gray-500">
                    <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-emerald-500 inline-block rounded" />YES probability</span>
                    <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 bg-red-500 inline-block rounded" />NO probability</span>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'info' && (
                <div className="rounded-xl border border-gray-800 bg-gray-900 p-5 space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-medium text-white mb-2">Settlement Rules</h4>
                      <p className="text-sm text-gray-400">This market will be automatically settled using on-chain data provided by Chainlink oracles within 24 hours after the closing date. If oracle data is unavailable, an administrator will settle the market manually.</p>
                    </div>
                  </div>
